...
public class MapBuilder {
    ...
    private static final int MAP_BUILDER_START_MENU = 0;
    private static final int MAP_BUILDER_ERASER_ENABLED = 1;
    private static int mapBuilderState = MAP_BUILDER_START_MENU;
    ...
    private boolean cellOrderNeedsUpdate = true;
            ...
            for (int i = 0; i < len; i++) {
                ...
                switch (mapBuilderState) {
                    case MAP_BUILDER_START_MENU:
                        updateMapBuilderMain(deltaTime, touchEvent);
                        break;
                    case MAP_BUILDER_ERASER_ENABLED:
                        updateEraserState(deltaTime, touchEvent);
                        break;
                }
            }
        }
    ...
    private void updateEraserState(float deltaTime, Input.TouchEvent touchEvent) {
    ...
    private void updateMapBuilderMain(float deltaTime, Input.TouchEvent touchEvent) {
        if (OverlapTester.pointInRectangle(eraser, touchPoint)){
            if (touchEvent.type == Input.TouchEvent.TOUCH_UP) {
                if (cellChosen == null) {
            ...
            if (touchEvent.type == Input.TouchEvent.TOUCH_UP) {
                ...
                if (cellFree) {
                    ...
                    if (cellChosen == CellContent.Maelstrom) {
                        ...
                        World.maelstrom.position.set(World.tileCenterCoords((int) touchPoint2.x), World.tileCenterCoords((int) touchPoint2.y));
                        lastTile.onMap = true;
                        lastTile = null;
                        World.fillWorldGrid();
                    }
